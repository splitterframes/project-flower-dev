...
import { Seed, PlantedField, UserFlower, UserButterfly, FieldButterfly, ExhibitionFrame } from '../types';
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  fetchGameData: async (userId: number) => {
    set({ loading: true });
    try {
      const [seedsRes, fieldsRes, flowersRes, butterfliesRes] = await Promise.all([
      ...
      const [seedsData, fieldsData, flowersData, butterfliesData] = await Promise.all([
        seedsRes.json(),
        fieldsRes.json(),
        flowersRes.json(),
        butterfliesRes.json(),
