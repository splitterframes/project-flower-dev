...
export const fieldFlowers = pgTable("field_flowers", {
  ...
  fieldIndex: integer("fieldIndex").notNull(), // 0-49 for the 50 garden fields, specifically pond fields
  flowerId: integer("flowerId").notNull(),
  flowerName: text("flowerName").notNull(),
  flowerRarity: text("flowerRarity").notNull(),
  flowerImageUrl: text("flowerImageUrl").notNull(),
  placedAt: timestamp("placedAt").notNull().defaultNow(),
  isShrinking: boolean("isShrinking").notNull().default(false), // Backend managed shrinking state for animation
