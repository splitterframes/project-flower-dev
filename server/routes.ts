...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Feed fish with butterfly (convert to caterpillar first) endpoint
  app.post('/api/garden/feed-butterfly', async (req, res) => {
    try {
      const { userId, butterflyId, fieldIndex } = req.body;

      if (!userId || !butterflyId || fieldIndex === undefined) {
        return res.status(400).json({ message: 'Missing required parameters' });
      }

      console.log('🦋→🐛 Converting butterfly', butterflyId, 'to caterpillar and feeding fish on field', fieldIndex);

      // Get user's butterfly to verify they have it
      const userButterflies = await storage.getUserButterflies(userId);
      const butterflyToUse = userButterflies.find(b => b.butterflyId === butterflyId);
      
      if (!butterflyToUse || butterflyToUse.quantity <= 0) {
        return res.status(400).json({ message: 'Du hast keine Schmetterlinge dieses Typs im Inventar.' });
      }

      // Remove one butterfly from inventory
      const removed = await storage.removeButterflyFromUser(userId, butterflyId, 1);
      
      if (!removed) {
        return res.status(400).json({ message: 'Fehler beim Entfernen des Schmetterlings aus dem Inventar.' });
      }

      // Convert butterfly rarity to caterpillar equivalent (same rarity system)
      const caterpillarRarity = butterflyToUse.butterflyRarity;

      // Track feeding progress (same system as caterpillars)
      const progressKey = `${userId}_${fieldIndex}`;
      const currentProgress = pondFeedingProgress.get(progressKey) || 0;
      const newProgress = currentProgress + 1;
      
      pondFeedingProgress.set(progressKey, newProgress);
      console.log('🐟 Butterfly->Caterpillar feeding result:', { fieldIndex, feedingCount: newProgress, fishCreated: newProgress >= 3 });
      ...
      console.error('Feed butterfly error:', error);
      res.status(500).json({ message: 'Fehler beim Füttern mit Schmetterling.' });
    }
  });
